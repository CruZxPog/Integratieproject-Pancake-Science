...
# later eventueel:
# from mqtt import publish_start_session
...
def api_sessions(program_id):
    ...
    if request.method == "POST":
        ...
        # programma naar Arduino sturen (voorbereiden)
...
def api_start_session(session_id):
    ...
    if user_id == 0:
        return jsonify({"status":"error","message":"user_id ontbreekt"}),400

    print("START SESSION", session_id, user_id)

    # later eventueel:
    # publish_start_session(user_id, session_id)
